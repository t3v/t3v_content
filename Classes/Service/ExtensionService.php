<?php
namespace T3v\T3vContent\Service;
 ...
 *
 * @package T3v\T3vContent\Service
 */
class ExtensionService extends AbstractService {
  /**
   * The modes which T3v Content supports.
   */
  const STRICT_MODE   = 'strict';
  const FALLBACK_MODE = 'fallback';

  /**
   * Checks if T3v Content is running in `strict` mode.
   *
   * @return bool If T3v Content is running in `strict` mode
   */
  ...
  /**
   * Checks if T3v Content is running in `fallback` mode.
   *
   * @return bool If T3v Content is running in `fallback` mode
   */
  ...
  /**
   * Gets the settings from `plugin.tx_t3vcontent.settings`.
   *
